...
#[derive(Clone,Copy,Debug,PartialEq,Eq)]
pub enum UnsupportedReason { FlexHead, BudgetExhausted, NotAProposition }
impl UnsupportedReason { pub const fn as_str(self)->&'static str{match self{Self::FlexHead=>"flex_head",Self::BudgetExhausted=>"budget_exhausted",Self::NotAProposition=>"not_a_proposition"}} }

impl Evidence {
    pub fn supports(&self)->Warrant{match self{
    ...
    pub fn tag(&self)->&'static str{match self{
        Self::LeanTheorem{..}=>"lean_theorem",Self::CanonicalEq{..}=>"canonical_eq",
        Self::AntiUnification{..}=>"anti_unification",Self::DependencyPath{..}=>"dependency_path",
        Self::RankingFeatures{..}=>"ranking_features",Self::Counterexample{..}=>"counterexample",
        Self::LeanAxiom{..}=>"lean_axiom",Self::Unsupported{..}=>"unsupported",
    }}
}
...
#[derive(Clone,Debug,PartialEq)]
pub struct Relation {
    pub left:String,pub right:String,pub kind:RelationKind,pub direction:Direction,
    pub evidence:Evidence,pub level:Option<String>,pub generator:String,pub schema_version:u32,
}
impl Relation {
    pub fn new(left:impl Into<String>,right:impl Into<String>,kind:RelationKind,direction:Direction,evidence:Evidence,generator:impl Into<String>)->Result<Self,RelationError>{
        let (needs,supports)=(kind.warrant(),evidence.supports());
        if supports>needs{return Err(RelationError::InsufficientEvidence{kind,needs,evidence:evidence.tag(),supports});}
        if kind.is_symmetric()!=(direction==Direction::Both){return Err(RelationError::DirectionMismatch{kind,direction});}
        Ok(Self{left:left.into(),right:right.into(),kind,direction,evidence,level:None,generator:generator.into(),schema_version:SCHEMA_VERSION})
    }
    pub fn at_level(mut self,level:impl Into<String>)->Self{self.level=Some(level.into());self}
    pub fn warrant(&self)->Warrant{self.kind.warrant()}
    pub fn explain(&self)->String{format!("{} {} {} [{}, {:?}] via {}",self.left,match self.direction{Direction::Both=>"~",Direction::LeftToRight=>"->",Direction::RightToLeft=>"<-"},self.right,self.kind,self.warrant(),self.evidence.tag())}
}
