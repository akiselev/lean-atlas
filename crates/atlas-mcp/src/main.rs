...
use atlas_daemon_protocol::{
    Command, DocumentRequest, LeanLaunch, OpenProjectRequest, ProjectMutationRequest, ProjectRequest,
};
...
fn launch(root: &str) -> Result<LeanLaunch, String> {
    ...
    Ok(LeanLaunch { program, args, root_uri })
}

fn client_error(error: ClientError) -> String {
    match error {
        ClientError::Remote(remote) => serde_json::to_string(&remote).unwrap_or_else(|_| format!("{remote:?}")),
...
async fn tool(client: &AtlasClient, name: &str, args: Value) -> Result<Value, String> {
    let payload = match name {
        "atlas.ping" => client.command(1, Command::Ping).await.map_err(client_error)?,
        "atlas.open_project" => {
            ...
            client
                .command(
                    ...
                    Command::OpenProject(OpenProjectRequest {
                        root: root.into(),
                        lean: launch(root)?,
                        store_path: args.get("store_path").and_then(Value::as_str).map(str::to_string),
                    }),
                )
                .await
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(line) = lines.next_line().await? {
        ...
        let response = match method {
            "initialize" => json!({"jsonrpc":"2.0","id":id,"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{}},"serverInfo":{"name":"atlas-live-mcp","version":env!("CARGO_PKG_VERSION")}}}),
            ...
            "tools/call" => {
                ...
                match tool(&client, name, args).await {
                    Ok(value) => json!({"jsonrpc":"2.0","id":id,"result":{"content":[{"type":"text","text":serde_json::to_string_pretty(&value)?}],"isError":false}}),
