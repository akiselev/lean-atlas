...
pub fn evaluate_optimized<S: FactSource>(
...
) -> Result<crate::Evaluation, LogicError> {
    ...
    for s in 0..=strata.values().copied().max().unwrap_or(0) {
        ...
        for round in 0..opts.max_rounds {
            ...
            for rule in p
                .rules
                .iter()
                .filter(|r| strata.get(&r.head.relation).copied().unwrap_or(0) == s)
            {
                let positives = rule
                    .body
                    .iter()
                    .enumerate()
                    .filter_map(|(i, l)| matches!(l, Literal::Pos(_)).then_some(i))
                    .collect::<Vec<_>>();
                let states = if round == 0 {
                    ...
                    let mut all = Vec::new();
                    for i in positives {
                        all.extend(body_delta(&db, &delta, &rule.body, i)?)
                    }
                    all
                };
            ...
            if produced.values().all(Vec::is_empty) {
                break;
            }
            for (r, fs) in &produced {
                db.entry(*r).or_default().extend(fs.iter().cloned())
            }
            delta = produced;
    ...
    rows.dedup();
    rows.truncate(q.limit.unwrap_or(opts.max_results).min(opts.max_results));
...
fn body_delta(
    ...
    delta: &BTreeMap<RelationTypeId, Vec<FactRow>>,
    lits: &[Literal],
    delta_at: usize,
    ...
    for (i, l) in lits.iter().enumerate() {
        match l {
            Literal::Pos(a) => {
                let rows = if i == delta_at {
                    delta.get(&a.relation)
                } else {
                    db.get(&a.relation)
                };
                let mut n = Vec::new();
                for (b, s) in states {
                    for f in rows.into_iter().flatten() {
                        if let Some(nb) = crate::source::bind_atom(&a.terms, &f.args, &b) {
                            let mut ns = s.clone();
                            ns.push(f.id);
                            n.push((nb, ns))
                        }
            ...
            Literal::Neg(a) => states.retain(|(b, _)| {
                !db.get(&a.relation)
                    .into_iter()
                    .flatten()
                    .any(|f| crate::source::bind_atom(&a.terms, &f.args, b).is_some())
            }),
            Literal::Eq(a, b) => {
                let mut out = Vec::new();
                for (mut bs, s) in states {
                    match (
                        crate::source::eval_term(a, &bs),
                        crate::source::eval_term(b, &bs),
                    ) {
                        ...
                        (None, None) => {
                            if matches!((a,b),(Term::Var(x),Term::Var(y)) if x==y) {
                                out.push((bs, s))
                            }
                        }
                        _ => {}
